      ...
      <div className='max-w-7xl mx-auto py-24'>
         ...
         <div className='mt-4'>
            {Object.entries(data.specs).map(([key, value]) => (
               <div key={key} className='py-4 sm:grid sm:grid-cols-3 sm:gap-4  border-b border-black'>
                  <dt className='text-xs tracking-tight font-medium uppercase'>{key}</dt>
                  <dd className='mt-1 text-xs tracking-tight sm:mt-0 sm:col-span-2'>{value}</dd>
               </div>
            ))}
         </div>
      </div>
